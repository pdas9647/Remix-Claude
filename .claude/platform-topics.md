...
**Field types for `ins` / `events`:** `string`, `number`, `bool`, `date`, `color`, `url`, `image`, `icon`, `ref`, `email`, `card`, `event`, `location`, `data`, `entity`, `binary`, `{}` (object), `["string"]` etc. (arrays)
...
```json
{
  ...
  "functions": [
    {
      "name": "qr_code",
      "arguments": [{"name": "url", "builder_type": "string"}],
      "builder_type_return": "binary",
      "doc": ["Generates QR code binary for a provided URL"]
    }
  ],
