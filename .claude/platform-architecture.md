...
- **mixquery** (Rust): Used by mixer (agent server), app clips, widgets, web components. Nearly feature-equivalent to eleventwo. Can operate on the same file hierarchy in parallel (but avoid concurrent access).
