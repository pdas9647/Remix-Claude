...
- `id` (UUID), `actual_labour_hours/cost` (combined totals), `actual_travel_hours/cost`, `total_hours_to_date`, `total_cost_to_date` (combined totals), `project_id/name` (always empty),
...
- `prl_payroll_employee_earning` — Earnings line items per employee. Has `earning_type` (REGULAR, OVERTIME, DOUBLE_OVERTIME), `amount`, `hours`, links to `project_id`, `cost_code_id`, `task_id`,
  `timesheet_id`
